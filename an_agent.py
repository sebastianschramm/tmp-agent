...
def clarification_rephraser(last_user_message: str, last_ai_response: str) -> str:
    ...
    clarification_prompt = [
        SystemMessage(
            "You are a communication and rephrasing expert. Rephrase the last AI message to clarify the users confusion."
        ),
        HumanMessage(
            f"AI said: {last_ai_response}. User asked: {last_user_message}. Provide a clearer rephrased AI message."
        ),
    ]
    llm = get_llm()
...
def get_llm(
    llm_name: str = "gemini-2.5-flash", tools: List[Callable] = None
...
if __name__ == "__main__":
    ...
    prompt = [
        AIMessage(
            "What are the main metrics you should consider for scaling your load based scaling on EKS?"
        ),
        HumanMessage("I don't understand what EKS means?"),
    ]
